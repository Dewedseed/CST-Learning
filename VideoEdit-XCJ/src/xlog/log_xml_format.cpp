...
static std::string TagXML(const std::string& str, const std::string& tag) {
    return "<" + tag + ">" + str + "</" + tag + ">";
}
...
///
std::string LogXMLFormat::Format(
    const std::string &level,
    const std::string &log,
    const std::string &file,
    int line
){
    return TagXML(TagXML(GetNow(), "time") +
                  TagXML(log, "log") +
                  TagXML(file, "file") +
                  TagXML(to_string(line), "line"),
                  level);
};
